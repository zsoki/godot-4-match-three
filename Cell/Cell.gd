...
func initialize(game_manager: GameManager, coord: Vector2i, position: Vector2,
		cell_size: int, selectable: bool):
	self.coord = coord
...
func _on_cell_input_event(viewport, event, shape_idx):
	if (event.is_action_pressed("select") and _selectable):
